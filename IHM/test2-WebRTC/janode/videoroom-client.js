...
async function doOffer(feed, display) {
  if (!pcMap.has(feed)) {
    ...
    let chanel = pc.createDataChannel('data');
    console.log('chanel', chanel);  
    chanel.onopen = () => console.log('chanel.onopen');
    chanel.onclose = () => console.log('chanel.onclose');
    chanel.onmessage = event => console.log('chanel.onmessage', event);
    chanel.onerror = event => console.log('chanel.onerror', event);
  
    
    
    pcMap.set(feed, pc);
    chanelMap.set(feed, chanel);
